<?php
  use App\Http\Controllers\FcrBeforeSaleFunctionController;
  $Function 	= new FcrBeforeSaleFunctionController();
?>
@extends('layout')
        ...
        <div class="col-md-12">
                ...
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <h3 class="p-3 mb-2 bg-primary text-white" style="padding:2px;">Fcr Before Sale</h3>
                    </div>
                </div>
                      ...
                      <button class="btn btn-danger" id="exportButton" onclick="createFormattingWorkbook()">Export to pdf</button>
                        <table class="table table-bordered table-hover" id="exportTable">
                          ...
                          <tr>
                              ...
                              <th>WEIGHT</th>
                              <th>TARGET WEIGHT</th>
                              <th>TOTAL FEED CONS.</th>
                              <th>FEED CONS. PER BIRD(gm)</th>
                              <th>TARGET FEED CONSUMTION</th>
                              <th>F.C.R WITH M</th>
                          ...
                          <tbody>
                            @foreach($endUsers as $eu)
                              <tr>
                                ...
                                <td>{{ $Function->getreporting_date($eu->id) }}</td>
                                ...
                                <td>{{ $Function->getmortality_percent($eu->id) }}</td>
                                <td>{{ $Function->getweight($eu->id) }}</td>
                                <td>{{ $Function->gettargetweight($eu->id) }}</td>
                                <td>{{ $Function->gettotal_feed_cons($eu->id) }}</td>
                                <td>{{ $Function->getfeed_cons_per_bird($eu->id) }}</td>
                                <td>{{ $Function->gettargetfeedconsumtion($eu->id) }}</td>
                                <td>{{ $Function->getfcr_with_m($eu->id) }}</td>
                                <td>{{ $Function->getfcr_without_m($eu->id) }}</td>
